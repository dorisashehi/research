  ...
  return (
    <div className="absolute top-5 right-5 max-w-96 max-h-[85vh] bg-slate-900/95 backdrop-blur-md rounded-xl border border-slate-700/50 shadow-2xl overflow-hidden z-50 flex flex-col">
      ...
      <div className="flex justify-between items-center p-5 border-b border-slate-700/50 bg-slate-800/50">
        <h3 className="text-2xl font-semibold text-white">{countryInfo.name}</h3>
        <button
      ...
      <div className="overflow-y-auto flex-1 p-5">
        ...
        {loading ? (
          ...
          <div>
            <div className="text-cyan-400 text-lg font-semibold mb-5 flex items-center gap-2">
                  ...
                  >
                    ...
                    >
                      <div className="flex gap-3 flex-1">
                        <span className="bg-gradient-to-b from-cyan-400 to-cyan-500 text-slate-900 font-bold text-sm px-3 py-1.5 rounded min-w-fit">
                          #{idx + 1}
                        </span>
                        <div className="flex-1">
                          <div className="font-semibold text-white text-sm leading-tight mb-1">{subfield.name}</div>
                      ...
                      {(hasTopics || (onFetchTopics && subfield.id)) && (
                        <span
                          className={`text-cyan-400 text-lg transition-transform ml-2 flex-shrink-0 ${
                        ...
                        {isLoading ? (
                          ...
                          <ul className="space-y-2.5">
                            {topics.map((topic, topicIdx) => (
                              ...
                              >
                                <span className="text-gray-300 flex-1">{topic.name}</span>
                                <span className="text-cyan-400 font-semibold ml-3 whitespace-nowrap">
